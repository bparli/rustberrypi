...
#define RX_STS_ERROR (RX_STS_FF | RX_STS_ES | RX_STS_LE | RX_STS_TL | RX_STS_CS | RX_STS_RW | RX_STS_ME | RX_STS_DB | RX_STS_CRC)
...
boolean SMSC951xDeviceConfigure(TUSBFunction *pUSBFunction)
{
	...
	u16 usMACAddressHigh = (u16)MACAddressBuffer[4] | (u16)MACAddressBuffer[5] << 8;
	u32 nMACAddressLow = (u32)MACAddressBuffer[0] | (u32)MACAddressBuffer[1] << 8 | (u32)MACAddressBuffer[2] << 16 | (u32)MACAddressBuffer[3] << 24;
	if (!SMSC951xDeviceWriteReg(pThis, ADDRH, usMACAddressHigh) || !SMSC951xDeviceWriteReg(pThis, ADDRL, nMACAddressLow))
	{
	...
	if (!SMSC951xDeviceWriteReg(pThis, LED_GPIO_CFG, LED_GPIO_CFG_SPD_LED | LED_GPIO_CFG_LNK_LED | LED_GPIO_CFG_FDX_LED) || !SMSC951xDeviceWriteReg(pThis, MAC_CR, MAC_CR_RCVOWN
...
boolean SMSC951xDevicePHYWrite(TSMSC951xDevice *pThis, u8 uchIndex, u16 usValue)
{
	...
	if (!SMSC951xDevicePHYWaitNotBusy(pThis) || !SMSC951xDeviceWriteReg(pThis, MII_DATA, usValue))
	{
		return FALSE;
	}
...
boolean SMSC951xDevicePHYRead(TSMSC951xDevice *pThis, u8 uchIndex, u16 *pValue)
{
	...
	u32 nMIIAddress = (PHY_ID_INTERNAL << 11) | ((u32)uchIndex << 6);
	u32 nValue;
	if (!SMSC951xDeviceWriteReg(pThis, MII_ADDR, nMIIAddress | MII_BUSY) || !SMSC951xDevicePHYWaitNotBusy(pThis) || !SMSC951xDeviceReadReg(pThis, MII_DATA, &nValue))
	{
		return FALSE;
	}
